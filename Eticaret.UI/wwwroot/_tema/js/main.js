$(document).ready(function () {
    ...
    function Last5Song(stationid) {
        $.ajax({
            url: "/Service/Last5Song",
            ...
            success: function (result)
            {
                var songlist = "";
                $('#lastsongs').html('');
                $.each(result, function (i, item) { songlist += "<li class='list-group-item'>" + item.ARTISTNAME + " - " + item.TRACKNAME + "</li>"; }); $('#lastsongs').append(songlist);
            },
            complete: function () { },
            error: function (xhr, status) {
                if (xhr.status == 401) { console.log(xhr.statusText); }
    ...
    Carousel
    ==============================*/
    $('.main__carousel--radio').owlCarousel({
        mouseDrag: true,
        touchDrag: true,
        dots: true,
        loop: true,
	...
	<div class="plyr__controls">
		...
		<div class="plyr__wrap">
			...
			<a asp-action="Index" asp-controller="radyolar" class="plyr__control" aria-label="Radyo Listesi">
				...
				<span class="plyr__tooltip" role="tooltip">Radyo Listesi</span>
			</a>
		</div>
	</div>	`;
